...
# 1. INSERISCI QUI TRA LE VIRGOLETTE LA TUA CHIAVE OTTENUTA DA ALPHA VANTAGE
CHIAVE_API = "CI005LE5TNX3RFLQ"

# 2. DOWNLOAD GLOBAL DATA TRAMITE PROXY UFFICIALE API (Nessun blocco IP del server)
@st.cache_data(ttl=60) # Aggiorna i dati ogni 60 secondi
def carica_dati_completi():
    # Ticker formattati per le API internazionali di Alpha Vantage
    tickers = {
        "STM_MILANO": "STM.MIL",
        "LEONARDO_MILANO": "LDO.MIL",
        "NVIDIA_USA": "NVDA",
        "AMD_USA": "AMD",
        "TSMC_TAIWAN": "TSM", # Utilizza la quotazione USA ad alta frequenza
        "NASDAQ_100": "QQQ",  # ETF Proxy del Nasdaq ad aggiornamento istantaneo
        "FTSEMIB": "EWI"      # ETF MSCI Italy per il trend macro europeo
    }
    ...
    for nome, tkr in tickers.items():
        try:
            # Chiamata API ufficiale approvata (Non aggirabile dai firewall di Streamlit)
            ...
            if "Global Quote" in dati_json and dati_json["Global Quote"]:
...
# 3. ALGORITMO QUANTITATIVO ADATTIVO (Analisi Micro-Trend e Flussi Globali)
...
st.write("Plancia di comando predittiva. Analisi delle correlazioni tra Piazza Affari, Wall Street e Taiwan tramite API Cloud.")
...
# Plancia di salvataggio di sicurezza (Se superi il limite di chiamate gratuite al minuto)
if prezzi.get("STM_MILANO", 0) == 0:
    st.warning("⚠️ Limite di chiamate API al minuto raggiunto (o mercato chiuso). Attivata simulazione tecnica.")
